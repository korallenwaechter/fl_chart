...
class FlDotIconPainter extends FlDotPainter {
  ...
  /// by setting the thickness with [strokeWidth],
  /// and you can change the color of of the stroke with [strokeColor].
  FlDotIconPainter({required Image image, required double height})
      : image = image,
        height = height;
  ...
  @override
  void draw(Canvas canvas, FlSpot spot, Offset offsetInCanvas) {
    // Offset newOffset = Offset(offsetInCanvas.dx - getSize(spot).width / 2,
    //     offsetInCanvas.dy - getSize(spot).height / 2);
    paintImage(
        canvas: canvas,
        image: image,
        rect: Rect.fromPoints(
            Offset(
                offsetInCanvas.dx - height / 2, offsetInCanvas.dy - height / 2),
            Offset(offsetInCanvas.dx + height / 2,
                offsetInCanvas.dy + height / 2)));
    // canvas.drawCircle(
    //     offsetInCanvas,
    //     3,
    //     Paint()
